...
const client =redis.createClient({
  password: "u5VhPaRLhQzPAOSJqhI4pugmZEgIgGzy",
  socket: {
      host: 'redis-11408.c212.ap-south-1-1.ec2.cloud.redislabs.com:11408',
      port: 11408
  }
});
client.on('connect', () => {
...
// mongoose.connect('mongodb+srv://Anjali:Anjali123@cluster0.yhrfanx.mongodb.net/test', { useNewUrlParser: true});
mongoose.connect('mongodb://localhost:27017/BMD', { useNewUrlParser: true});
  ...
  Registers.find({},(err,results) => {
      if (err) {
          res.status(500).send(err);
           res.json({auth:false,results:null,fromCache: isCached});
        }
        else{
        res.send(results)
